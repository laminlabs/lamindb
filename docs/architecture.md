...
Transferring data from one database to another has three noteworthy properties:
...
- it functions like a **sync**: due to idempotency of operations in LaminDB, a repeated transfer does not duplicate data
...
- **Git-like branching (Write-Audit-Publish):** LaminDB provides database-level branching (`stage`, `review`, `merge`) to support isolated workflows, similar to tools like git or Nessie.[^nessie] Rather than spinning up a new isolated metadata database clone, the underlying database and its schema remain shared. Isolation is achieved in two ways: all metadata objects in LaminDB live on a branch by virtue of `SQLRecord`'s `branch` field. This ensures that _new_ objects are isolated, but it does not isolate changes to metadata objects. All versioned objects — artifacts, collections, transforms, and blocks (notes) — also isolate changes. That is, if you make a change to an artifact or note block that change is isolated on a branch. Hence, agents or developers can safely perform exploratory writes or pipeline runs on a dedicated branch without corrupting production datasets, transforms, or notes. Metadata and schema updates of non-versioned entities, however, apply globally. For more, see {doc}`manage-changes`.
