...
Note that artifact keys in Lamin do **not** mirror the local directory layout
exactly — the callback uses the Lamin run UID instead of Lightning's
...
Lamin artifact keys are **not** a mirror of the local filesystem layout.
...
Instead, when `ln.track()` is active, the callback uses the **Lamin run UID**
