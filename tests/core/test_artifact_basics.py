...
def test_revise_recreate_artifact(example_dataframe: pd.DataFrame, ccaplog):
    ...
    artifact = ln.Artifact.from_dataframe(
        df, key=key, description="test", _version_tag="1"
    )
    assert artifact._version_tag == "1"
    ...
    with pytest.raises(ValueError) as error:
        artifact_v2 = ln.Artifact.from_dataframe(df, revises=artifact, _version_tag="1")
    assert (
        error.exconly()
    ...
    assert artifact_v2.uid.endswith("0001")
    assert artifact_v2.stem_uid == artifact.stem_uid
    assert artifact_v2._version_tag is None
    ...
    assert artifact._version_tag is None
    ...
    assert new_artifact._version_tag is None
