...
def test_create_from_local_filepath(
    ...
    if key is None and not is_in_registered_storage and description is None:
        ...
        assert error.exconly() == (
            f"lamindb.errors.InvalidArgument: The passed path suffix '{suffix}' must match the passed key suffix '{key_suffix}'."
        )
        return None
...
def test_df_suffix(df):
    ...
    assert (
        ...
        == "lamindb.errors.InvalidArgument: The passed key suffix '.def' must match the passed path suffix '.parquet'."
    )
...
def test_adata_suffix(get_small_adata):
    ...
    assert (
        error.exconly().partition(",")[0]
        == "lamindb.errors.InvalidArgument: The passed key suffix '' must match the passed path suffix '.h5ad'."
    )
