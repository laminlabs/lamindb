...
def test_artifact_set_values_preserves_dataset_features():
    ...
    before_ids = {
        ln.Artifact: set(ln.Artifact.filter().values_list("id", flat=True)),
        ln.Record: set(ln.Record.filter().values_list("id", flat=True)),
        ln.Feature: set(ln.Feature.filter().values_list("id", flat=True)),
        ln.Schema: set(ln.Schema.filter().values_list("id", flat=True)),
        bt.CellType: set(bt.CellType.filter().values_list("id", flat=True)),
        bt.Gene: set(bt.Gene.filter().values_list("id", flat=True)),
    }
    ...
    try:
        ...
        dataset_feature_keys = set(artifact.load().obs.columns) & set(
            values_before.keys()
        )
        assert len(dataset_feature_keys) > 0
        dataset_values_before = {
            key: values_before[key] for key in dataset_feature_keys
        }
        ...
        for model in [
            ...
            current_ids = set(model.filter().values_list("id", flat=True))
            new_ids = current_ids - before_ids[model]
            if new_ids:
                model.filter(id__in=new_ids).delete(permanent=True)
