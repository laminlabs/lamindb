...
def test_revise_collection(df, adata):
    ...
    collection = ln.Collection(artifact, key="test-collection", _version_tag="1")
    assert collection._version_tag == "1"
    ...
    with pytest.raises(ValueError) as error:
        collection_r2 = ln.Collection(artifact, revises=collection, _version_tag="1")
    assert (
        error.exconly()
    ...
    assert collection_r2.stem_uid == collection.stem_uid
    assert collection_r2.uid.endswith("0001")
    assert collection_r2._version_tag is None
    ...
    assert collection_r3._version_tag == "2"
