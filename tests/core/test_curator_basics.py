...
def test_curators_df_nested_cat():
    ...
    feature1 = ln.Feature(name="biosample_id", dtype=str).save()
    feature2 = ln.Feature(name="biosample_name", dtype=sample_a_type).save()
    ...
    with pytest.raises(ln.errors.ValidationError) as error:
        curator.validate()
    assert (
        "2 terms not validated in feature 'biosample_name': 'sample3', 'sample4'"
        in error.exconly()
    )
