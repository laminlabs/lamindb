...
from lamindb._parse import parse

df = pd.DataFrame(
    (["T cell", "CL:0000084"], ["hepatocyte", "CL:0000182"], ["my new cell type", ""]),
    columns=["cell_type", "cell_type_id"],
)


def test_parse_name():
    result = parse(df.cell_type, CellType.name)
    ids = [i.ontology_id for i in result]
    assert len(result) == 3
    assert set(ids) == set(["CL:0000182", None, "CL:0000084"])


def test_parse_ontology_id():
    result = parse(df.cell_type_id, CellType.ontology_id)
