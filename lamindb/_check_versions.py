...
if version.parse(lnschema_core_v) != version.parse("0.33rc1"):
    raise RuntimeError("lamindb needs lnschema_core==0.33rc1")

if version.parse(lndb_storage_v) != version.parse("0.2rc4"):
    raise RuntimeError("lamindb needs lndb_storage==0.2rc4")

if version.parse(lndb_v) < version.parse("0.41.0"):
    raise RuntimeError("Upgrade lndb! pip install lndb==0.41.0")
