...
if version.parse(lndb_setup_v) != version.parse("0.30.6"):
    raise RuntimeError("lamindb needs lndb_setup==0.30.6")

if version.parse(lnschema_core_v) != version.parse("0.24.1"):
    raise RuntimeError("lamindb needs lnschema_core==0.24.1")

if version.parse(bionty_v) != version.parse("0.6.2"):
    raise RuntimeError("lamindb needs bionty==0.6.2")
