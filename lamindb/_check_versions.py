...
if version.parse(lndb_setup_v) < version.parse("0.4.3"):
    raise RuntimeError("lamindb needs lndb_setup >= 0.4.3")

if version.parse(lndb_hub_v) < version.parse("0.3.1"):
    raise RuntimeError("lamindb needs lndb_hub >= 0.3.1")

if version.parse(lndb_schema_core_v) < version.parse("0.2.0"):
    raise RuntimeError("lamindb needs lndb_schema_core >= 0.2.0")
