...
class Schema(SQLRecord, HasType, CanCurate, TracksRun, TracksUpdates):
    ...
    Args:
        ...
        index: `Feature | None = None` Index feature for row keys. For `DataFrame` / `AnnData` curation, validates `df.index` or `obs` / `var` indices. On record sheets, stored on :attr:`~lamindb.Record.name`; see :class:`~lamindb.Record`.
    ...
    def index(self) -> None | Feature:
        ...
        For record sheet schemas, see :class:`~lamindb.Record`. To unset, set
        `schema.index` to `None`.
        """
