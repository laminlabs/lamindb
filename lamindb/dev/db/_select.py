...
def _create_select_func(model):
    ...
    def select_func(settings_store: InstanceSettingsStore = None, **kwargs):
        return _select(model=model, kwargs=kwargs, settings_store=settings_store)
...
def _select(
    model, result_list=None, settings_store: InstanceSettingsStore = None, kwargs=dict()
) -> SelectResult:
    """Simple queries."""
...
def select_dobject(
    ...
    return _select(model=model, result_list=results, settings_store=settings_store)
