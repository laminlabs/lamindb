...
def install_ci(session, group):
    ...
    if group in ["unit-core-sqlite", "unit-core-postgres"]:
        ...
        run(session, "uv pip install --system spatialdata setuptools")
        ...
        run(
            session,
            "uv pip install --system scanpy mudata spatialdata tiledbsoma setuptools",
        )
    elif group == "biology":
        extras += "fcs"
        ...
        run(
            session,
            "uv pip install --system spatialdata setuptools",
        )
        run(session, "uv pip install --system tiledbsoma")
