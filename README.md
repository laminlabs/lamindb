...
- **unified access:** query any number of storage locations (local, S3, GCP, https, HF, R2), SQL databases (Postgres, SQLite), ontologies, and workflow managers
- **biology aware:**
  - _data structures:_ validate, auto-annotate & query formats like `DataFrame`, `AnnData`, `MuData`, ... backed by `parquet`, `zarr`, HDF5, LanceDB, DuckDB, ...
  - _entities:_ organize features & labels in registries for experimental metadata & extendable ontologies
- **findable, queryable & batch-loadable:** manage schemas to impose integrity for distributed queries & batch loading
